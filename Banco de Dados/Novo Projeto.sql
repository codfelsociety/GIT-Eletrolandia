...
BEGIN
  ...
  COMMIT;
END;
EXECUTE insert_imagem_produto (161, 8575, 'ddd') ;
...
SELECT prod.cod_produto AS cod,  img.imagem, prod.nome, art.descricao AS artigo, cat.descricao AS categoria,
                           mar.descricao AS marca, prod.estoque,logprod.custo, logprod.preco_venda AS preco, 
                            (logprod.preco_venda - logprod.custo) * prod.estoque AS lucro_total, prod.ativo AS status,prod.data_cadastro AS data
                            FROM produto prod
                            JOIN artigo art
                            ...
                            SELECT img.imagem, prod.nome from imagem_produto img
                            JOIN produto prod
                            ON prod.cod_produto = img.cod_produto;